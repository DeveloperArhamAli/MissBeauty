...
const getProductsForHomePage = async function (req, res){
    try {
        let products = await productModel.find();
        res.send({
...
const getProduct = async function (req, res) {
    try {
        ...
        const productWithBase64Images = {
            ...product._doc, // Get product data
            image: product.image.map((img) =>
                img ? `data:image/png;base64,${img.toString("base64")}` : null
            ),
        };
        res.send({
            randomProducts, productWithBase64Images
        });
    } catch (error) {
